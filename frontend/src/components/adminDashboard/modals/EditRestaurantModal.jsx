...
const EditRestaurantModal = ({ isOpen, onClose, restaurant }) => {
  ...
  // Load existing restaurant data when modal opens
 useEffect(() => {
  if (restaurant) {
    ...
    if (restaurant.managerImage) {
      setManagerImagePreview(restaurant.managerImage);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.keys(resturantData).forEach((key) =>
        updateFormData.append(key, resturantData[key])
      );
      if (managerImageFiles) {
        updateFormData.append("managerImage", managerImageFiles);
      }
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6">
              <form className="h-full flex flex-col" onSubmit={handleSubmit}>
                {/* 🔁 Reuse the same steps as AddRestaurantModal (Basic Info, Manager, Location, Images) */}
                {/* Just copy the JSX of each step from AddRestaurantModal */}
                {/* ... */}
